		...
		public function add_args( $args ) {
			...
			if ( $license ) {
				$args['license-info'][ $current_theme_slug ] = array(
					...
					/*
					 * Send both plaintext 'license' and hashed 'license-hash'.
					 * The remote API is still being updated to accept 'license-hash'
					 * (the new, secure format). Sending both fields keeps the
					 * plugin working with both old and new API versions. Once the
					 * remote side is fully migrated, the 'license' field can be removed.
					 */
					'license'     => $license,
					'url'         => home_url(),
