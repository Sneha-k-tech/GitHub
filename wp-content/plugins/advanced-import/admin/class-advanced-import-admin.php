...
class Advanced_Import_Admin {
	...
	public function template_button( $item ) {
		if ( $this->is_template_available( $item ) ) {
			$button  = '<a class="button ai-demo-import ai-item-import is-button is-default is-primary is-large button-primary" href="#" aria-label="' . esc_attr__( 'Import', 'advanced-import' ) . '"';

			if ( isset( $item['theme']['slug'] ) && advanced_import_get_current_theme_slug() !== $item['theme']['slug'] ) {
				$button .= ' data-theme="' . esc_attr( wp_json_encode( $item['theme'] ) ) . '"';
			}
			if ( isset( $item['plugins'] ) && is_array( $item['plugins'] ) ) {
				$button .= ' data-plugins="' . esc_attr( wp_json_encode( $item['plugins'] ) ) . '"';
			}

			$button .= '><span class="dashicons dashicons-download"></span>' . esc_html__( 'Import', 'advanced-import' ) . '</a>';
		} else {
			$button  = '<a class="button is-button is-default is-primary is-large button-primary"';
			$button .= ' href="' . esc_url( isset( $item['pro_url'] ) ? $item['pro_url'] : '#' ) . '"';
			$button .= ' target="_blank"';
			$button .= ' aria-label="' . esc_attr__( 'View Pro', 'advanced-import' ) . '">';
			$button .= '<span class="dashicons dashicons-awards"></span>' . esc_html__( 'View Pro', 'advanced-import' ) . '</a>';
		}

		$render_button = apply_filters( 'advanced_import_template_import_button', $button, $item );
	...
	public function wp_enqueue_styles() {
		$asset = require ADVANCED_IMPORT_PATH . 'build/index.asset.php';
		wp_enqueue_style( $this->plugin_name, ADVANCED_IMPORT_URL . 'build/index.css', array( 'wp-admin', 'dashicons' ), $asset['version'], 'all' );
		wp_enqueue_media();
	}
	...
	public function wp_enqueue_scripts() {
		...
		wp_enqueue_script( $this->plugin_name, ADVANCED_IMPORT_URL . 'build/index.js', array_merge( $asset['dependencies'], array( 'jquery' ,'masonry') ), $asset['version'], true );
		wp_localize_script(
			$this->plugin_name,
			'advanced_import_object',
			array(
				'ajaxurl' => admin_url( 'admin-ajax.php' ),
				'wpnonce' => wp_create_nonce( 'advanced-import' ),
				'text'    => array(
	...
	/**
	 * Validate that unzipped content contains only JSON files and an uploads folder.
	 *
	 * @access private
	 * @param array|null $dirlist Directory listing from WP_Filesystem.
	 * @return bool True if valid, false otherwise.
	 */
	private function validate_unzipped_content( $dirlist ) {
	...
	public function import_menu() {
		$this->hook_suffix[] = add_theme_page( esc_html__( 'Demo Import ', 'advanced-import' ), esc_html__( 'Demo Import', 'advanced-import' ), 'manage_options', 'advanced-import', array( $this, 'demo_import_screen' ) );
	...
	public function get_header() {
		...
		$welcome_msg .= '</div>';
		echo wp_kses_post( apply_filters( 'advanced_import_welcome_message', $welcome_msg ) );
	...
	public function upload_zip() {
		if ( isset( $_FILES['ai-upload-zip-archive']['name'] ) && ! empty( $_FILES['ai-upload-zip-archive']['name'] ) ) {
			...
			$upload_zip_archive = $_FILES['ai-upload-zip-archive']; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
			...
			if ( ! $this->validate_unzipped_content( $dirlist ) ) {
				$wp_filesystem->rmdir( ADVANCED_IMPORT_TEMP, true );
				wp_send_json_error(
					array(
	...
	public function demo_download_and_unzip() {
		...
		if ( $demo_file_type == 'file' ) {
			...
			$max_size     = (int) apply_filters( 'advanced_import_attachment_size_limit', 0 );
			$request_args = array(
				'timeout'            => 30,
				'redirection'        => 3,
			...
			<div class="ai-filter-tabs">
				<ul class="ai-types ai-filter-group" data-filter-group="secondary">
					...
					foreach ( $unique_types as $cat_index => $single_type ) {
						?>
						<li class="ai-filter-btn ai-type-filter" data-filter=".<?php echo esc_attr( strtolower( $single_type ) ); ?>">
							<?php echo esc_html( ucfirst( $single_type ) ); ?>
							<span class="ai-count"></span>
		...
		<div class="ai-filter-content" id="ai-filter-content">
						...
						if ( is_array( $categories ) && ! empty( $categories ) ) {
							foreach ( $categories as $demo_index => $demo_cats ) {
								foreach ( $demo_cats as $cat_index => $single_cat ) {
									...
									?>
									<li class="ai-filter-btn" data-filter=".<?php echo esc_attr( strtolower( $single_cat ) ); ?>">
			...
			<div class="ai-filter-content-wrapper">
				...
				foreach ( $demo_lists as $key => $demo_list ) {
					...
					"
						<?php
						if ( $this->is_template_available( $demo_list ) ) {
							echo $data_template . ' ' . $data_template_type; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
						}
						?>
					>
						<?php
						...
						<div class="ai-item-footer">
							<div class="ai-item-footer_meta">
								...
								<div class="ai-item-footer-actions">
									...
									<?php
									echo $this->template_button( $demo_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
									?>
								</div>
								<?php
								...
								if ( ! empty( $keywords ) ) {
									...
									foreach ( $keywords as $cat_index => $single_keywords ) {
										?>
										<li class="<?php echo esc_attr( strtolower( $single_keywords ) ); ?>"><?php echo esc_html( ucfirst( $single_keywords ) ); ?></li>
										<?php
									}
									echo '</ul>';
								}
		...
		<div class="ai-form <?php echo $total_demo > 0 ? 'hidden' : ''; ?>">
			<form action="" method="post" enctype="multipart/form-data" id="ai-upload-zip-form">
				...
				<div class="input-file"><input type="file" name="ai-upload-zip-archive" id="ai-upload-zip-archive" size="50" /></div>
					...
					echo esc_html( sprintf( __( 'Maximum upload file size: %s', 'advanced-import' ), size_format( wp_max_upload_size() ) ) );
					?>
				</p>
	...
	public function theme_screen() {
		...
		/*check for security*/
		check_admin_referer( 'advanced-import' );
		$theme_info_raw = isset( $_POST['themeInfo'] ) ? wp_unslash( $_POST['themeInfo'] ) : ''; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		if ( is_array( $theme_info_raw ) ) {
			$theme_info = $theme_info_raw;
		} elseif ( is_string( $theme_info_raw ) ) {
			$theme_info = (array) json_decode( $theme_info_raw, true );
		...
		$theme_info = array_map( 'sanitize_text_field', $theme_info );
		...
		<div class="ai-notification-title">
			...
			printf( esc_html__( 'We are installing the %s theme...', 'advanced-import' ), esc_html( $theme_info['title'] ) );
			?></p>
		</div>
	...
	* */
	...
	public function plugin_screen() {
		...
		<ul class="ai-plugins-wrap hidden">
			<?php
			$recommended_plugins_raw = isset( $_POST['recommendedPlugins'] ) ? wp_unslash( $_POST['recommendedPlugins'] ) : ''; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
			if ( is_array( $recommended_plugins_raw ) ) {
				$recommended_plugins = $recommended_plugins_raw;
			} elseif ( is_string( $recommended_plugins_raw ) ) {
				$recommended_plugins = (array) json_decode( $recommended_plugins_raw, true );
			...
			$recommended_plugins = array_map( function( $plugin ) {
				return is_array( $plugin ) ? array_map( 'sanitize_text_field', $plugin ) : sanitize_text_field( (string) $plugin );
			}, $recommended_plugins );
	...
	private function advanced_import_setup_content_steps() {
		...
		*/
		...
		$content = apply_filters( 'advanced_import_' . $this->theme_name . '_theme_view_setup_step_content', $content );
	...
	public function content_screen() {
		...
		if ( isset( $_POST['template_url'] ) ) {
			$template_url = wp_unslash( $_POST['template_url'] ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
			if ( is_array( $template_url ) ) {
				$this->current_template_url = array_map( 'sanitize_url', $template_url );
			} else {
				$this->current_template_url = sanitize_url( $template_url );
			}
			$this->current_template_type = isset( $_POST['template_type'] ) ? sanitize_text_field( wp_unslash( $_POST['template_type'] ) ) : '';
		}
		...
		<div class="ai-notification-title">
			<p>
				<?php
				esc_html_e( 'It\'s time to insert some demo content for your new WordPress Site. Once inserted, this content can be managed from the WordPress admin dashboard. ', 'advanced-import' )
				?>
			</p>
		</div>
	...
	*/
	public function import_content() {
		/*check for security*/
		if ( ! check_ajax_referer( 'advanced-import', 'wpnonce' ) ) {
			wp_send_json_error(
				array(
					'message' => esc_html__( 'Security check failed.', 'advanced-import' ),
				)
			);
		}
		...
		$content_slug = isset( $_POST['content'] ) ? sanitize_title( wp_unslash( $_POST['content'] ) ) : '';

		$content = $this->advanced_import_setup_content_steps();

		/*check for security again*/
		if ( ! check_ajax_referer( 'advanced-import', 'wpnonce' ) || empty( $content_slug ) || ! isset( $content[ $content_slug ] ) ) {
			wp_send_json_error(
				array(
					'error'   => 1,
				...
				if ( $result ) {
					...
					if ( is_array( $result ) && isset( $result['retry'] ) ) {
						...
						$json = array(
							...
							'_wpnonce'      => wp_create_nonce( 'advanced-import' ),
	...
	private function import_content_post_type_data() {
		...
		$post_type = ! empty( $_POST['content'] ) ? sanitize_text_field( wp_unslash( $_POST['content'] ) ) : false;
		...
		$limit = 10 + ( isset( $_REQUEST['retry_count'] ) ? absint( wp_unslash( $_REQUEST['retry_count'] ) ) : 0 );
	...
	/**
	 * Set and get imported post IDs from transient.
	 *
	 * @since 1.0.0
	 * @param int|string|false $original_id Original post ID.
	 * @param int|string|false $new_id      New post ID to map.
	 * @return int|string|array|false Mapped ID, array of all IDs, or false.
	 */
	...
	/**
	 * Set and get post orphans/post parent mapping from transient.
	 * If parent is not already imported, the child will be orphaned.
	 *
	 * @since 1.0.0
	 * @param int|string|false $original_id       Original post ID.
	 * @param int|string|false $missing_parent_id Missing parent ID to map.
	 * @return int|string|array|false Mapped parent ID, array of all orphans, or false.
	 */
	...
	/**
	 * Set delayed post for later processing.
	 *
	 * @since 1.0.0
	...
	/**
	 * Import a single post or content item.
	 *
	 * @since 1.0.0
	 * @param string $post_type Post type slug.
	 * @param array  $post_data Post data array.
	 * @param int    $delayed   Whether this is a delayed import (0 or 1).
	...
	private function process_import_single_post( $post_type, $post_data, $delayed = 0 ) {
		...
		if ( ! empty( $post_data['post_title'] ) && ! empty( $post_data['post_name'] ) ) {
			global $wpdb;
			$cache_key = 'ai_post_lookup_' . md5( $post_data['post_name'] . '|' . $post_data['post_title'] . '|' . $post_type );
			$pages     = wp_cache_get( $cache_key, 'advanced_import' );
			if ( false === $pages ) {
				$pages = $wpdb->get_results( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery
					$wpdb->prepare(
						"SELECT ID, post_name, post_parent, post_type FROM {$wpdb->posts} WHERE post_name = %s AND post_title = %s AND post_type = %s",
						array(
							$post_data['post_name'],
				...
				wp_cache_set( $cache_key, $pages, 'advanced_import' );
			}
		...
		switch ( $post_type ) {
			...
			case 'attachment':
				...
				if ( isset( $post_data['guid'] ) && ! empty( $post_data['guid'] ) ) {
					...
					$post_data['upload_date'] = gmdate( 'Y/m', strtotime( $post_data['post_date_gmt'] ) );

					if ( isset( $post_data['meta'] ) ) {
						foreach ( $post_data['meta'] as $key => $meta ) {
							if ( $key == '_wp_attached_file' ) {
								foreach ( (array) $meta as $meta_val ) {
									if ( preg_match( '%^[0-9]{4}/[0-9]{2}%', $meta_val, $date_matches ) ) {
										$post_data['upload_date'] = $date_matches[0];
									}
								}
							}
						}
					...
					if ( $attach_id ) {
						...
						if ( file_exists( $file_path ) && is_readable( $file_path ) ) {
							...
							trigger_error( esc_html( "Attachment file missing or unreadable: $file_path" ), E_USER_WARNING );
						}
	...
	*/
	...
	private function import_image_and_file( $url, $post ) {
		...
		if ( is_file( $local_file ) && filesize( $local_file ) > 0 ) {
			...
			$upload    = wp_upload_bits( $file_name, null, $file_data, $post['upload_date'] );
			if ( $upload['error'] ) {
	...
	*/
	 ...
	 * @access public
	 *
	 * @param string $string Check if valid JSON.
	 * @return boolean
	 */
	...
	public function complete_screen() {
		...
		/*check for security*/
		if ( ! check_ajax_referer( 'advanced-import', 'wpnonce' ) ) {
			wp_die( esc_html__( 'Security check failed.', 'advanced-import' ) );
		}
	...
	* */
	...
	/**
	 * Add help text to the current screen.
	 *
	 * @since 1.0.0
	...
	public function help_tabs( $screen ) {
		...
		$screen->add_help_tab(
			array(
				...
				'content' =>
					...
					'<p>' . sprintf(
						/* translators: %s: link to Advanced Export plugin */
					...
					'<p>' . sprintf(
						/* translators: %1$s: link to Advanced Export, %2$s: link to Advanced Import */
		...
		$screen->add_help_tab(
			array(
				...
				'content' =>
					'<h2><strong>' . __( 'WordPress Reset', 'advanced-import' ) . '</strong></h2>' .
		...
		ob_end_clean();
		if ( $has_admin_notice ) {
			$has_admin_notice = '<h2><strong>' . __( 'Allow Track', 'advanced-import' ) . '</strong></h2>' . $has_admin_notice;
			$screen->add_help_tab(
				array(
