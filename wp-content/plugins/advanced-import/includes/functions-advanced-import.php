...
function advanced_import_add_log( $entry, $file = 'advanced-import.log', $mode = 'default' ) {
	...
	// Use WP_CONTENT_DIR instead of uploads directory to avoid public exposure.
	...
	$existing = '';
	if ( 'default' === $mode || 'a' === $mode ) {
		if ( $wp_filesystem->exists( $file_path ) ) {
			$existing = $wp_filesystem->get_contents( $file_path );
		}
	}

	$new_entry = $existing . $entry . "\n";
	if ( ! $wp_filesystem->put_contents( $file_path, $new_entry, FS_CHMOD_FILE ) ) {
		return 0;
	}
...
/**
 * Get a WordPress post by its title.
 *
 * @since 1.0.0
 * @param string $title     Post title.
 * @param string $post_type Post type slug. Default 'page'.
 * @return WP_Post|false WP_Post object on success, false on failure.
 */
...
if ( ! function_exists( 'advanced_import_install_plugin' ) ) {
	...
	function advanced_import_install_plugin( $plugin_info ) {
		...
		if ( is_wp_error( $result ) ) {
			...
			return array(
				'code'    => $skin->get_errors()->get_error_code(),
				'success' => false,
				'message' => sprintf(
					/* translators: %1$s is the plugin name, %2$s is error message */
					esc_html__( 'Error installing plugin "%1$s": %2$s', 'advanced-import' ),
					esc_html( $name ),
					esc_html( $skin->get_errors()->get_error_message() )
				),
			);
