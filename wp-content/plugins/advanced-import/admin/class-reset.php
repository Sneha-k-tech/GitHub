...
class Advanced_Import_Reset_WordPress {
	...
	private function can_reset() {
		if ( ! empty( $_GET['ai_reset_wordpress'] ) && ! empty( $_GET['ai_reset_wordpress_nonce'] ) ) {
			/*Security*/
			if ( ! wp_verify_nonce( wp_unslash( $_GET['ai_reset_wordpress_nonce'] ), 'ai_reset_wordpress' ) ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
				return false;
			}
	...
	public function hide_reset_notice() {
		if ( isset( $_GET['advanced-import-hide-notice'] ) && isset( $_GET['_advanced_import_notice_nonce'] ) ) {
			/*Security*/
			if ( ! wp_verify_nonce( wp_unslash( $_GET['_advanced_import_notice_nonce'] ), 'advanced_import_hide_notice_nonce' ) ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
				wp_die( esc_html__( 'Action failed. Please refresh the page and retry.', 'advanced-import' ) );
			}

			if ( ! current_user_can( 'manage_options' ) ) {
				wp_die( esc_html__( 'Cheatin&#8217; huh?', 'advanced-import' ) );
			}

			$hide_notice = sanitize_text_field( wp_unslash( $_GET['advanced-import-hide-notice'] ) );
	...
	public function reset_wizard_actions() {
		...
		if ( ! empty( $_GET['ai_reset_wordpress'] ) && ! empty( $_GET['ai_reset_wordpress_nonce'] ) && $this->can_reset() ) {
			...
			// Drop tables.
			$drop_tables = $wpdb->get_col( $wpdb->prepare( "SHOW TABLES LIKE %s", $wpdb->esc_like( $wpdb->prefix ) . '%' ) ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching
			if ( ! empty( $drop_tables ) ) {
				foreach ( $drop_tables as $table ) {
					if ( 0 === stripos( $table, $wpdb->prefix ) ) {
			...
			wp_safe_redirect( $current_url . '&reset=true&from=ai-reset-wp&_wpnonce=' . wp_create_nonce( 'advanced-import' ) );
			exit();
		}
	}
	...
	public function reset_wizard_notice() {
		...
		if ( ! $demo_notice_dismiss ) {
			...
			<div id="message" class="updated ai-import-message">
				<p><strong><?php esc_html_e( 'WordPress Reset', 'advanced-import' ); ?></strong> &#8211; <?php esc_html_e( 'If no important data on your site. You can reset the WordPress back to default again!', 'advanced-import' ); ?></p>
			...
			<?php
		} elseif ( isset( $_GET['_wpnonce'] ) && wp_verify_nonce( sanitize_key( wp_unslash( $_GET['_wpnonce'] ) ), 'advanced-import' ) && isset( $_GET['reset'] ) && 'true' === sanitize_text_field( wp_unslash( $_GET['reset'] ) ) ) {
			$user = get_user_by( 'id', 1 );
			?>
			<div id="message" class="notice notice-info is-dismissible">
				<p><?php
				/* translators: %s: username */
				echo esc_html( sprintf( __( 'WordPress has been reset back to defaults. The user "%1$s" was recreated with its previous password.', 'advanced-import' ), $user->user_login ) );
				?></p>
			</div>
			<?php
		}
